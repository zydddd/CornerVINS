/*
 * RVG-VIO: VIO System construct by NLPR RVG Group, Institute of Automation, CAS
 *
 * Author: Fulin Tang, Hao Wei, Yihong Wu
 * Email: fulin.tang@nlpr.ia.ac.cn
 *        weihao2019@ia.ac.cn
 *
 * Created by weihao on 2021/1/19.
 */

#ifndef RGBD_PLANE_VIO_UPDATE_SLAM_FEATURE_H
#define RGBD_PLANE_VIO_UPDATE_SLAM_FEATURE_H

#include <Eigen/Eigen>
#include "ekf/state.h"
#include "ekf/state_utils.h"
#include "point_features/point_feature.h"
#include "data_types/landmark.h"
#include "point_features/point_triangulation.h"
#include "quat_utils.h"

#include "update_utility.h"

#include <random>

namespace Rvg {

/**
 * @brief Will compute the system for our sparse SLAM features and update the filter.
 */
class UpdateSlamFeature
{
public:


    /**
     * @brief Default constructor for our SLAM updater
     */
    UpdateSlamFeature(UpdaterParameters &parameters_, PointTriangulationParameters &feat_init_parameters, std::map<int, double>& chi_square);


    /**
     * @brief Given tracked SLAM features, this will try to use them to update the state.
     */
    void Update(std::shared_ptr<State> state, std::vector<std::shared_ptr<PointFeature>>& feature_vec);


    /**
     * @brief Given max track features, this will try to use them to initialize them in the state.
     */
    void DelayedInit(std::shared_ptr<State> state, std::vector<std::shared_ptr<PointFeature>>& feature_vec);
    void DelayedInitOri(std::shared_ptr<State> state, std::vector<std::shared_ptr<PointFeature>>& feature_vec);


    /**
     * @brief Will change SLAM feature anchors if it will be marginalized
     */
    void ChangeAnchors(std::shared_ptr<State> state);


protected:
    double ComputeMAD(const Eigen::VectorXd &res);
    /**
     * @brief Shifts landmark anchor to new clone
     */
    void PerformAnchorChange(std::shared_ptr<State> state, std::shared_ptr<Landmark> landmark, double new_anchor_timestamp);


    /// Options used during update for slam features
    UpdaterParameters parameters_;

    /// Feature initializer class object
    std::unique_ptr<PointTriangulation> initializerFeat_;

    /// Chi squared 95th percentile table (lookup would be size of residual)
    std::map<int, double> chiSquaredTable_;

    double maxOptIter_ = 3;
    std::ofstream debug_file;
};
}

#endif RGBD_PLANE_VIO_UPDATE_SLAM_FEATURE_H
